...
A single server process running on a single machine has a ceiling: however many requests one process can handle at once is the app's entire capacity. Get more customers than that, and the fix usually isn't a bigger building — it's more of them.
...
## Seeing the loop in code

Here's that listen-read-respond loop with nothing hidden, in both languages — the same route from the API post, `GET /restaurants/42/menu`, with no framework in the way.
